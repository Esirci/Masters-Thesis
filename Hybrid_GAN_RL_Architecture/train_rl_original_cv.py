"""
train_rl_original_cv.py
------------------------
Trains the DQN agent on the ORIGINAL (non-augmented, imbalanced) fold data.
Used as a comparison baseline against the GAN-augmented pipeline.

Usage:
    python train_rl_original_cv.py --fold 1
    (trains on cv_splits/fold_1_train.csv,
     evaluates on cv_splits/fold_1_test.csv,
     saves to cv_results_original/fold_1_results.json)
"""
...
def main(fold: int):
    ...
    train_path = os.path.join(SPLITS_DIR, f"fold_{fold}_train.csv")
    test_path  = os.path.join(SPLITS_DIR, f"fold_{fold}_test.csv")
    model_path = os.path.join(MODELS_DIR, f"fold_{fold}_dqn_original")

    print(f"\n=== Fold {fold}: RL on Original (Imbalanced) Data ===")
    ...
    # Build env DataFrame expected by TrojanEnv
    ...
    obs, _ = env.reset()
    assert obs.shape[0] == X_train_scaled.shape[1]
    ...
    print(f"\nFold {fold} Results (Original Data):")
