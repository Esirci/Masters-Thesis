            ...
            for input_pin in gate.getInputPins():
                for wire in input_pin.connected_wires.keys():
                    ...
                    if wire.id not in data:
                        ...
                        data[wire.id].extend([max(1e-7, (1-input_pin.PROB_1)) if input_pin.PROB_1 is not None else 0.5, max(1e-7, input_pin.PROB_1) if input_pin.PROB_1 is not None else 0.5])
            ...
            for output_pin in gate.getOutputPins():
                for wire in output_pin.connected_wires.keys():
                    ...
                    if wire.id not in data:
                        ...
                        data[wire.id].extend([max(1e-7, (1-output_pin.PROB_1)) if output_pin.PROB_1 is not None else 0.5, max(1e-7, output_pin.PROB_1) if output_pin.PROB_1 is not None else 0.5])
